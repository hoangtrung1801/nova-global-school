import campusHero from './images/ChatGPT Image 23_43_42 26 thg 6, 2026.png'
import learningHall from './images/ChatGPT Image 23_44_43 26 thg 6, 2026.png'
import libraryImage from './images/ChatGPT Image 23_44_48 26 thg 6, 2026.png'
import classroomImage from './images/ChatGPT Image 23_44_58 26 thg 6, 2026.png'
import sportsImage from './images/ChatGPT Image 23_45_48 26 thg 6, 2026.png'
import aerialImage from './images/ChatGPT Image Jun 26, 2026, 11_34_43 PM.png'
import contentPdf from './section-content.pdf'

export const siteContent = {
  title: 'NOVA GLOBAL SCHOOL',
  subtitle:
    'Trường liên cấp THCS - THPT tại Hòa Xuân, Cẩm Lệ, Đà Nẵng.',
  statement:
    'Nova Global School kết hợp chương trình học thuật, tiếng Anh tăng cường và cơ sở vật chất hiện đại để chuẩn bị cho học sinh năng lực hội nhập.',
  ctas: [
    { label: 'Đăng ký tư vấn tuyển sinh', href: '#lien-he' },
    { label: 'Xem khuôn viên trường', href: '#thu-vien' },
  ],
  heroImage: campusHero,
  overview: {
    location:
      'Nova Global School nằm trên đường Đá Bàn 7, phường Hòa Xuân, quận Cẩm Lệ, Đà Nẵng; kết nối nhanh với Võ Chí Công, Quốc lộ 1A và các khu đô thị phía Nam thành phố.',
    developer:
      'Dự án được phát triển theo định hướng của Tập đoàn Nguyễn Hoàng (NHG), tập trung vào mô hình trường tư thục hiện đại và vận hành bền vững.',
    scale:
      'Trường phục vụ học sinh 11 - 17 tuổi ở hai cấp THCS và THPT, với công suất tối đa 920 học sinh trên khu đất 21.390 m².',
    stats: [
      { value: '21.390 m²', label: 'Tổng diện tích đất quy hoạch' },
      { value: '12.643 m²', label: 'Tổng diện tích sàn xây dựng' },
      { value: '920', label: 'Học sinh công suất tối đa' },
      { value: '11 - 17', label: 'Độ tuổi đào tạo trọng tâm' },
    ],
  },
  academic: [
    {
      title: 'Sự giao thoa chuẩn mực',
      body: 'Chương trình kết hợp khung MOET với định hướng đánh giá năng lực, giúp học sinh vững kiến thức và sẵn sàng chuyển tiếp.',
    },
    {
      title: 'Ngoại ngữ là chìa khóa mở cửa thế giới',
      body: 'Tiếng Anh tăng cường cùng giáo viên bản ngữ hỗ trợ học sinh hướng tới IELTS hoặc VSTEP B2/C1 khi tốt nghiệp.',
    },
    {
      title: 'Triết lý phát triển 5 nền tảng',
      body: 'Lộ trình học cân bằng học thuật, thể chất, tư duy phản biện, cảm xúc - đạo đức và kỹ năng thế kỷ 21.',
    },
  ],
  facilities: [
    {
      title: 'Khối lớp học và hành chính hiệu bộ',
      body: 'Khối 4 tầng và 1 tầng tum, diện tích 10.559 m², gồm phòng học lấy sáng tự nhiên, bảng tương tác và điều hòa trung tâm.',
    },
    {
      title: 'Khối nhà bộ môn và thí nghiệm chuyên sâu',
      body: 'Khu 3 tầng, diện tích 1.062 m², tích hợp phòng thí nghiệm, Âm nhạc, Hội họa, STEM Lab và Robot học.',
    },
    {
      title: 'Khu liên hợp thể thao ngoài trời và đường chạy',
      body: 'Hai sân bóng đá cỏ nhân tạo và đường chạy 853,5 m² tạo không gian rèn luyện thể chất ngoài trời.',
    },
    {
      title: 'Khối nhà đa năng',
      body: 'Nhà đa năng 1.022 m² có sàn thi đấu PVC, sân khấu và khán đài cho sự kiện học đường.',
    },
    {
      title: 'Hệ thống hậu cần và an toàn nội khu',
      body: 'Nhà ăn 336 m², hạ tầng kỹ thuật riêng, camera giám sát và lộ trình xe buýt hỗ trợ an toàn học đường.',
    },
  ],
  tuition: [
    {
      title: 'Học phí khối THCS',
      value: '63 triệu VNĐ / năm học',
      body: 'Khoảng 7 triệu VNĐ mỗi tháng.',
    },
    {
      title: 'Học phí khối THPT',
      value: '81 triệu VNĐ / năm học',
      body: 'Khoảng 9 triệu VNĐ mỗi tháng.',
    },
    {
      title: 'Chi phí đồng phục',
      value: '1,5 triệu VNĐ / năm',
      body: 'Bao gồm 2 bộ đồng phục học thuật, 1 bộ đồ thể thao và 1 áo khoác nhận diện thương hiệu.',
    },
  ],
  incentives: [
    'Lộ trình học phí ổn định, mức tăng dự kiến không quá 5% mỗi năm.',
    'Giảm 5% khi phụ huynh đóng học phí nguyên năm trước hạn.',
    'Ưu đãi tới 10% cho gia đình có từ con thứ hai hoặc học sinh đạt học bổng đầu vào.',
  ],
  hr: {
    stats: [
      { value: '88', label: 'Tổng nhân sự toàn thời gian' },
      { value: '46', label: 'Giáo viên đứng lớp' },
      { value: '1 / 10', label: 'Tỷ lệ nhân sự đồng hành với học sinh' },
    ],
    groups: [
      {
        title: 'Ban Giám Hiệu',
        body: '04 nhân sự gồm 01 Hiệu trưởng chịu trách nhiệm chiến lược chung và 03 Phó Hiệu trưởng chuyên trách: Chuyên môn MOET, Chương trình Quốc tế - Ngoại khóa, và Vận hành - Dịch vụ học đường.',
      },
      {
        title: 'Khối Chuyên môn Giảng dạy',
        body: '47 nhân sự, trong đó có 42 giáo viên bộ môn đạt chuẩn sư phạm, 03 nhân sự giáo vụ quản lý học bạ và thời khóa biểu, cùng 02 nhân sự thư viện phụ trách trung tâm học liệu.',
      },
      {
        title: 'Khối Quốc tế và Chương trình Ngoại khóa',
        body: '06 nhân sự gồm 04 giáo viên bản ngữ có chứng chỉ TESOL/CELTA và 02 chuyên viên kỹ năng sống chuyên trách thiết kế hoạt động ngoại khóa, dã ngoại sinh thái.',
      },
      {
        title: 'Khối Vận hành và Dịch vụ Đời sống',
        body: '23 nhân sự bao gồm y tế, tham vấn tâm lý học đường, đội bếp dinh dưỡng, quản lý xe đưa đón, kỹ thuật bảo trì tòa nhà, IT, an ninh và vệ sinh nội khu.',
      },
      {
        title: 'Khối Tham mưu và Hỗ trợ Quản lý',
        body: '08 nhân sự vận hành các đầu mối kế toán - tài chính, tuyển sinh - phát triển thương hiệu và quản trị nhân sự, đảm bảo dòng chảy thông tin cũng như tài chính của toàn trường.',
      },
    ],
  },
  finance: {
    capital: [
      { label: 'Tổng mức đầu tư dự án', value: '152,68 tỷ VNĐ' },
      { label: 'Nguồn vốn chủ sở hữu', value: '92,68 tỷ VNĐ · 60,7%' },
      { label: 'Nguồn vốn tín dụng tài trợ', value: '60,00 tỷ VNĐ · 39,3%' },
    ],
    metrics: [
      { label: 'Giá trị hiện tại thuần (NPV)', value: '+50,11 tỷ VNĐ' },
      { label: 'Suất sinh lời nội bộ (IRR)', value: '13,0%' },
      { label: 'Tỷ suất lợi ích / chi phí (B/C)', value: '1,095' },
      { label: 'Thời gian hoàn vốn', value: '10 năm 10 tháng' },
      { label: 'Hệ số trả nợ dài hạn (DSCR)', value: '3,247' },
    ],
    social:
      'Trong 15 năm vận hành, dự án dự kiến đóng góp 130,13 tỷ VNĐ vào ngân sách và hưởng ưu đãi xã hội hóa giáo dục theo quy định.',
  },
  scenarios: [
    {
      name: 'Rất Xấu',
      assumption: 'Công suất -30%, học phí -10%, chi phí +10%',
      npv: '-33,21 tỷ',
      irr: '7%',
      payback: '> 20 năm',
      status: 'Không khả thi',
      summary:
        'Kịch bản xấu nhất cho thấy dự án cần chính sách khóa biểu phí lộ trình và duy trì tỷ lệ lấp đầy ở ngưỡng an toàn ngay từ năm đầu tiên để tránh rơi vào vùng không khả thi.',
    },
    {
      name: 'Xấu',
      assumption: 'Công suất -15%, học phí 0%, chi phí +5%',
      npv: '+8,45 tỷ',
      irr: '11%',
      payback: '13 năm 8 tháng',
      status: 'Khả thi an toàn',
      summary:
        'Ngay cả khi sĩ số hụt 15% và chi phí tăng 5%, dự án vẫn duy trì IRR cao hơn chi phí sử dụng vốn 9%, cho thấy dư địa vận hành vẫn an toàn.',
    },
    {
      name: 'Kế Hoạch',
      assumption: 'Vận hành chuẩn theo các thông số gốc',
      npv: '+50,11 tỷ',
      irr: '13%',
      payback: '10 năm 10 tháng',
      status: 'Đạt kỳ vọng',
      summary:
        'Đây là kịch bản mục tiêu của nhà trường, cân bằng giữa tỷ lệ lấp đầy, mức học phí và chi phí vận hành để đạt hiệu quả kinh tế bền vững.',
    },
    {
      name: 'Tốt',
      assumption: 'Công suất +5%, học phí +15%, chi phí -5%',
      npv: '+64,02 tỷ',
      irr: '14%',
      payback: '9 năm 11 tháng',
      status: 'Vượt chỉ tiêu',
      summary:
        'Khi chiến dịch tuyển sinh đạt hiệu quả tốt và vận hành được tối ưu, dự án tạo ra biên độ tài chính tốt hơn đáng kể so với kế hoạch cơ sở.',
    },
    {
      name: 'Rất Tốt',
      assumption: 'Công suất +15%, học phí +20%, chi phí -10%',
      npv: '+91,81 tỷ',
      irr: '15%',
      payback: '8 năm 2 tháng',
      status: 'Tối ưu tuyệt đối',
      summary:
        'Kịch bản tăng trưởng cao cho thấy dự án còn dư địa mở rộng giá trị nếu thương hiệu được lan tỏa mạnh và tỷ lệ tuyển sinh vượt mục tiêu.',
    },
  ],
  downloads: {
    title: 'Hồ sơ dự án Nova Global School',
    body: 'Tải hồ sơ PDF để xem bản vẽ, chỉ tiêu tài chính, cơ cấu nhân sự và thông tin triển khai chi tiết.',
    href: contentPdf,
  },
  gallery: [
    { src: campusHero, alt: 'Phối cảnh tổng thể khuôn viên Nova Global School' },
    { src: learningHall, alt: 'Không gian sinh hoạt và học tập mở' },
    { src: libraryImage, alt: 'Thư viện và trung tâm học liệu' },
    { src: classroomImage, alt: 'Phòng học tiêu chuẩn hiện đại' },
    { src: sportsImage, alt: 'Không gian thể thao và hoạt động ngoại khóa' },
    { src: aerialImage, alt: 'Góc nhìn tổng quan khu đất từ trên cao' },
  ],
  footer: {
    legal: 'ĐỒ ÁN TỐT NGHIỆP - KHOA QUẢN LÍ DỰ ÁN - NGÀNH KINH TẾ XÂY DỰNG',
    project: 'DỰ ÁN ĐẦU TƯ XÂY DỰNG CÔNG TRÌNH NOVA GLOBAL SCHOOL',
    location: 'Đường Đá Bàn 7, Phường Hòa Xuân, Quận Cẩm Lệ, Thành phố Đà Nẵng, Việt Nam.',
    advisor: 'TS. Trương Ngọc Sơn',
    student: 'Đinh Quốc Tuấn',
    copyright:
      '© 2026 Dự án đầu tư xây dựng Nova Global School. Đồ án tốt nghiệp - Khoa Quản lý dự án - Đại Học Bách Khoa Đà Nẵng.',
  },
} as const
